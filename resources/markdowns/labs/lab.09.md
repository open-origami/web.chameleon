...
[Download](../../resources/downloads/labs/lab.09.zip)
     [Run](../../resources/workshops/labs/lab.09)

### Create the SLean Subject

Create a class with two methods to control lean effect. The code is similar to that in the Lab 7 but log sentences are not explicit. Use the Subject extension model and assign a namespace to it with the Point decorator.
...
Create a class with two methods to control zoom effect. The code is similar to that in the Lab 7 but log sentences are not explicit. Use the Subject extension model and assign a namespace to it with the Point decorator.
...
Create now a class with two methods to control min effect. The code is similar to that in the Lab 7 but log sentences are not explicit. Use the Subject extension model and assign a namespace to it with the Point decorator.
...
To randomly select subject controlled effects one more subject must be created. The STimer extension defines two methods to start & stop effects execution. The code is exactly the same to that in Lab 7.
...
To manage the trace logic an Aspect based extension can be created. The aspect is installed over the other extensions in the project and thus the log feature is applied by interception before each method execution.
...
Define a simple html file as an entry point for your project. Just include a div element working as our view component and needed buttons to control the effects on the view.
...
<html lang="en">
  ...
  <body>
    ...
    <h2>Lab 9. Aspect & Before Interception </h2>
    <p>
      In a previous example a Mixin was defined to manage trace operations.
      However the most suitable way to extends method behavior is by means of
      the Aspect Extension Model. Here an ALog is defined. Open console and check
      trace messages.
    </p>
